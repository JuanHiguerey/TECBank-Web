...
export const Citas = (props) => {
    ...
    const onRequestAppointment = (event) => {
        ...
        fetch(`http://localhost:1337/api/cita/${selectedDay.value}/${selectedTime.value}/${selectedLocation.value}`)
        .then(async response => {
            const data = await response.json();
            if (data.status === "success"){
                if (data.json.length === 0){
                    fetch(`http://localhost:1337/api/cita/${userId}/${selectedDay.value}/${selectedTime.value}/${selectedType.value}/${selectedLocation.value}`, {
                        method: 'POST'
                    })
                    .then(async response => {
                        const data = await response.json();
